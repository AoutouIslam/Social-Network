...
***********************************************************/
	if (!isset($_POST['x']) && !isset($_FILES['image']['name']) ){
		//Delete users temp image
			$temppath = 'Assets/images/profile_pics/"'.$userLoggedIn.'"/'.$profile_id.'_temp.jpeg';
...
***********************************************************/	
	...
	//Set Upload directory    
		...
		$createfolder = mkdir($_SERVER['DOCUMENT_ROOT'].'/Social Network/assets/images/profile_pics/'.$userLoggedIn .'',777);
		$uploaddir = $_SERVER['DOCUMENT_ROOT'].'/Social Network/assets/images/profile_pics/'.$userLoggedIn .'';
	//Set File name	
	...
	//Move the file to correct location
		...
		if (!$move) { 
			...
			$imgSrc= "Assets/images/profile_pics/$userLoggedIn/".$file_name; // the image to display in crop area
...
if (isset($_POST['x'])){
	...
	//the file type posted
		$type = $_POST['type'];	
	//the image src
		$src = 'Assets/images/profile_pics/'.$userLoggedIn.'/'.$_POST['src'];	
	...
	if($type == 'jpg' || $type == 'jpeg' || $type == 'JPG' || $type == 'JPEG'){	
		...
		//save the new cropped version
			imagejpeg($dst_r, "Assets/images/profile_pics/$userLoggedIn/".$finalname."n.jpeg", 90); 	
		...
		$result_path ="Assets/images/profile_pics/$userLoggedIn/".$finalname."n.jpeg";
